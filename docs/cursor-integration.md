...
**v0.2.10 新增同会话补触发**：同一任务多轮迭代中，若你指出 Agent "又错了 / 还是错 / 我刚说过 / 第 N 次"，或引用 Active Constraints 编号 `C{n}`，Phase 4 **不等下一轮新任务**就立即触发 rating=1，并在 `reflections.md` 追加反思骨架。

---

## 会话约束栈（v0.2.10）

**解决的问题**：同会话内你反复强调的格式/风格/技术约束，在 Agent 后续输出中被忽略。

**工作方式**：

1. 你在对话里说了硬性约束，比如：
   - "**必须**用 MD 格式，代码块和正文分开"
   - "我**反复**强调过，命名用 kebab-case"
   - "输出结构**不能**嵌套超过 3 层"
2. Agent 把它加入本会话的 Active Constraints 清单（心里记，不落盘）
3. 之后每次生成输出前，Agent 内部跑一遍约束自检
4. 若再违反，你点破（"又搞错了" / "C2 违反了"），Agent：
   - 简短致歉并修正（不辩解）
   - 本轮 Phase 4 强制 rating=1（写入 reflections.md）

**注意**：
- 清单只活在**本会话**，新会话不记忆（跨会话持久化由 Layer 2 承担，尚未实现）
- 最多维护 10 条，超出时最早的被替换（除非已被你点名过——"hot" 约束永久保留）
- Agent 不会主动展示清单；你可以问"你目前记住了哪些约束"来查看
