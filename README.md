...
| 想做什么 | 往哪走 |
|---------|--------|
| 想快速接入 Cursor？ | [Cursor 集成指南](docs/cursor-integration.md) |
...
复制 `cursor-rule/skillforge.mdc` 到项目的 `.cursor/rules/`，重启后自动生效。

详见 → [Cursor 集成指南](docs/cursor-integration.md)
...
```bash
python3 tests/test_skillforge.py       # 8 个核心端到端测试
python3 tests/test_mar.py              # 9 个 MAR 模块测试
python3 tests/test_vector_search.py    # 10 个向量检索测试
python3 tests/test_reflexion.py        # 13 个 Reflexion 单元测试
python3 tests/test_stage4_integration.py  # 11 个集成测试
# 共 51 个，全部通过
```
