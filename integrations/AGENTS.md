...
**解决的问题**：同会话内用户反复强调的格式/风格/技术约束在后续输出中被忽略。

**机制**：Agent 在工作记忆中维护 **Active Constraints** 清单（仅本会话，不落盘）。

**加入约束的触发信号**（任一命中）：
- **硬性命令**：「必须」「不能」「禁止」「一定要」「千万别」
- **重复强调**：「说过」「强调过」「反复」「每次都」「我说了几次」
- **明确格式/风格要求**：点名具体领域（MD 格式、代码块、命名、语气等）

**强制自检**：每次生成输出前，Agent 内部走一遍 Active Constraints 清单，违反则重写。

**被用户抓违反时**：回复开头简短致歉 + 点出违反的 `C{n}` 编号，**不辩解**，直接修正；本轮 Phase 4 强制 rating=1。

**清单规模**：最多 10 条；会话结束整体废弃（跨会话持久化由 Layer 2 承担，尚未实现）。

### Phase 4：质量评估（对用户透明）

**触发时机**：
- 主路径：下一轮用户消息进来时回溯识别上一轮
- **同会话补触发（v0.2.10）**：同一任务多轮迭代中，若用户反馈含"又错了/还是错/我刚说过/第 N 次/都说了"等重复犯错信号，或明确引用 Active Constraints 的 `C{n}` 编号，**不等下一轮新任务**，在本轮末尾立即静默执行 Phase 4（rating=1）

1. 扫描用户反馈判断 rating（默认 3；批评或违反已有约束为 1；明确惊喜为 5）
