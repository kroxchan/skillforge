...
| 许可证 | Apache 2.0 |
| 当前版本 | **v0.2.10** — 同会话纠错体系（Layer 1 + Layer 3） |
| 测试基线 | **159 passed**（本版本零代码，仅改 mdc + doc） |
| 最后更新 | 2026-04-23 |
...
- [版本记录](#版本记录)
  ...
  - [v0.2.10 (2026-04-23)](#v0210-2026-04-23--同会话纠错体系-layer-1--layer-3)
...
### v0.2.10 (2026-04-23) — 同会话纠错体系 (Layer 1 + Layer 3)

**状态**: 完成 | 测试 159 passed（零代码，只改 mdc + doc）
**触发**: 纯使用模式第二次真实数据暴露——用户反馈"同会话内强调过几次的 MD 格式约束，Agent 第二次输出还是违反"。排查发现 `reflections.md` 自 2026-04-15 建文件起**完全空白**，L0 九条记录全部 rating=3，即"同会话多轮纠错"这个最常见场景从未触发过任何学习机制

#### 根因诊断（三层失效）

| 层级 | 问题 | 本轮是否修 |
|------|------|----------|
| **Layer 1** | 同会话内 Phase 4 不触发，rating=1 永远识别不到 | ✅ 本版本 |
| **Layer 2** | 即使反思记录了，下次新会话 Agent 不会读 | ⏳ 等 L1 有数据后做 |
| **Layer 3** | 用户强调的约束在本会话后续输出中无任何持久化机制 | ✅ 本版本 |

根本上这不是 bug 而是**设计空白**——v0.2.x 所有版本都假设"一轮任务 = 一次评估"，完全没考虑同任务多轮迭代。

#### 核心变更

**Layer 3：会话约束栈（Constraint Stack）**

- mdc 新增 `### 会话约束栈` 章节（插在 Phase 3 之后、Forger 之前）
- Agent 在工作记忆维护 Active Constraints 清单（仅本会话，不落盘）
- **加入触发**：用户说"必须/不能/禁止/一定要"（硬性命令）或"说过/强调过/反复/每次都"（重复强调）或点名具体格式/风格领域
- **生命周期**：`active`（首次）→ `hot`（用户再次强调，违反即 rating=1）→ `dismissed`（用户明确解除）
- **强制自检**：Agent 每次生成输出前内部跑一遍，违反则重写
- **被抓违反时**：致歉 + 点出 `C{n}` 编号，**不辩解**，直接修正
- 最多 10 条，超出时替换最早 `active`；`hot` 永不删

**Layer 1：Phase 4 同会话补触发**

- rating=1 触发条件新增"重复犯错信号"：「又错了」「还是错」「我刚说过」「第 N 次」「都说了」，或明确引用 `C{n}` 编号
- Phase 4 原本只在"新任务边界"触发；现扩展为**同会话多轮迭代中检测到重复犯错信号 → 本轮末尾立即静默执行 rating=1**，不等下一轮新任务
- 目的：让 `reflections.md` 真正累积"反复被纠正"的失败数据，为 Layer 2 准备原材料
- 反思模板 root cause 维度从 3 条扩展到 4 条，新增「我是否在输出前完成了约束自检？」专门应对约束违反场景

**文档同步**

- `integrations/AGENTS.md`、`docs/cursor-integration.md`：新增"会话约束栈"小节 + Phase 4 补触发说明
- `SKILL.md`：更新日志追加 v0.2.10

#### 设计决策：为什么分两轮不一次做完

用户选了"三层全修"。但按 v0.2.9-patch 的教训：**一次性设计大而全，容易设计出"从未被真实数据验证"的机制**。

本版本只做 L1 + L3（零代码，全 mdc），原因：
- L1 必须先做：否则 reflections.md 永远为空，Layer 2 就没有原材料可读
- L3 最痛：直接解决用户这次反馈的核心场景
- L2 依赖 L1 数据：先让 L1 跑一周真实数据，再判断是否真需要跨会话读反思

写入 TODO：等 L1 累计 3~5 条真实 rating=1 反思数据，再做 L2（`sf recall` CLI + Phase 3 注入）。

#### 设计洞察

- **触发点缺失 > 算法不足**：SkillForge 过去所有优化都在"评估公式/权重/粒度"，但真正的问题是**评估根本没被触发过**。类似软件工程里"日志级别正确但 logger 初始化失败"。
- **会话工作记忆 vs 持久记忆分离**：Layer 3 刻意不落盘，是承认"用户在本会话的临时约束"和"需要跨会话继承的教训"是两件事，不该用同一套机制。
- **纯使用模式的 ROI 在继续升高**：v0.2.9-patch 用 7 次真实数据发现 P1；v0.2.10 用 9 次数据 + 一次真实反馈发现更深的 P1。**真实使用反馈的信噪比远高于主观 review**。

#### 触发下次 review 的条件（更新）
1. 累计 20+ 次真实 task（目前 9 次）
2. Forger 首次真实触发（某 task_type 达 count ≥ 5）
3. **reflections.md 累计 ≥ 3 条真实反思**（Layer 1 数据阈值，触发 Layer 2 设计）
4. 2 周时间到（约 2026-05-07）
5. 遇到使用层新问题
